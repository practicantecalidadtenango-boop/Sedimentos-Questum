import streamlit as st
import pandas as pd
import plotly.express as px
from datetime import datetime

# ==========================================
# 1. CONFIGURACIÓN DE PÁGINA Y ESTILOS
# ==========================================
st.set_page_config(page_title="Dashboard SPC Live - Questum", layout="wide")

# Lectura directa desde el repositorio
RUTA_BD = "BD_LECTURAS_LIVE.xlsx"

COLUMNAS_METADATA = [
    'Bot_Fecha', 'Bot_Hoja', 'Bot_Linea', 
    'N° Medición', 'Fecha y hora', 
    'TELESIS - NÚMERO DE TELESIS', '#_Parte - Número de Parte del Producto', 
    'Capturador'
]

# Límites predefinidos según tus especificaciones
LIMITES_PREDEFINIDOS = {
    "15-100": {"lsl": 0, "usl": 20000},
    "100-200": {"lsl": 0, "usl": 1200},
    "200-600": {"lsl": 0, "usl": 250},
    "600-1500": {"lsl": 0, "usl": 6},
    ">1500": {"lsl": 0, "usl": 0}
}

@st.cache_data(ttl=60)
def cargar_datos():
    try:
        df = pd.read_excel(RUTA_BD)
        df['Fecha y hora'] = pd.to_datetime(df['Fecha y hora'], errors='coerce')
        df['Bot_Fecha'] = pd.to_datetime(df['Bot_Fecha'], errors='coerce')
        return df
    except Exception as e:
        return None

# ==========================================
# 2. PROCESAMIENTO DE DATOS
# ==========================================
df = cargar_datos()

if df is not None:
    # FILTROS LATERALES
    st.sidebar.header("⚙️ Configuración")
    lista_lineas = sorted(df['Bot_Linea'].dropna().unique())
    linea_sel = st.sidebar.selectbox("Seleccionar Línea:", lista_lineas)
    
    df_linea = df[df['Bot_Linea'] == linea_sel]
    lista_hojas = sorted(df_linea['Bot_Hoja'].dropna().unique())
    hoja_sel = st.sidebar.selectbox("Seleccionar Operación:", lista_hojas)

    df_final = df_linea[df_linea['Bot_Hoja'] == hoja_sel].copy()
    
    # --- OBTENER ÚLTIMA ACTUALIZACIÓN ---
    # Usamos la fecha que el bot grabó al momento de subir los datos
    ultima_act = df_final['Bot_Fecha'].max()
    formato_fecha = ultima_act.strftime("%d/%m/%Y %H:%M:%S") if not pd.isna(ultima_act) else "Sin datos"

    # ==========================================
    # 3. LETREROS DE ENCABEZADO (NUEVO)
    # ==========================================
    # Creamos dos columnas para los letreros superiores
    c1, c2 = st.columns(2)
    
    with c1:
        st.info(f"🏗️ **LÍNEA ACTUAL:** {linea_sel}  \n🎯 **OPERACIÓN:** {hoja_sel}")
        
    with c2:
        st.success(f"📅 **ÚLTIMA ACTUALIZACIÓN DEL BOT:** \n⏰ {formato_fecha}")

    st.markdown("---")

    # DETECCIÓN DE VARIABLES
    columnas_vars = [c for c in df_final.columns if c not in COLUMNAS_METADATA and not c.startswith('Unnamed')]
    
    if columnas_vars:
        variable_sel = st.sidebar.selectbox("Variable a graficar:", columnas_vars)
        
        # BÚSQUEDA DE LÍMITES AUTOMÁTICOS
        def_lsl, def_usl = 0.0, 100.0
        for clave, limites in LIMITES_PREDEFINIDOS.items():
            if clave in variable_sel:
                def_lsl, def_usl = float(limites["lsl"]), float(limites["usl"])
                break
        
        # AJUSTE DE ZONAS EN SIDEBAR
        with st.sidebar.expander("🛠️ Ajustar Límites Visuales"):
            lsl = st.number_input("Límite Inferior (LSL)", value=def_lsl, format="%.1f")
            usl = st.number_input("Límite Superior (USL)", value=def_usl, format="%.1f")
            margen = st.slider("Margen de Advertencia %", 0, 50, 15) if usl > 0 else 0
            warning_upper = usl - ((usl - lsl) * margen / 100) if usl > 0 else 0

        # LIMPIEZA Y GRÁFICA
        df_final[variable_sel] = pd.to_numeric(df_final[variable_sel], errors='coerce')
        df_grafica = df_final.dropna(subset=[variable_sel, 'Fecha y hora']).sort_values('Fecha y hora')

        if not df_grafica.empty:
            # KPI'S
            prom = df_grafica[variable_sel].mean()
            val_maximo = df_grafica[variable_sel].max()
            
            k1, k2, k3, k4 = st.columns(4)
            k1.metric("Muestras", len(df_grafica))
            k2.metric("Media", f"{prom:.2f}")
            k3.metric("Máximo", f"{val_maximo:.2f}", delta="ALERTA" if val_maximo > usl else "OK", delta_color="inverse")
            k4.metric("Límite Superior", f"{usl:.1f}")

            # GRÁFICA SPC
            st.subheader(f"📈 Tendencia: {variable_sel}")
            fig = px.line(df_grafica, x='Fecha y hora', y=variable_sel, markers=True,
                         hover_data=['N° Medición', 'Capturador'], template="plotly_dark")

            # Dibujar Zonas
            tope = max(usl * 1.2, val_maximo * 1.1) if usl > 0 or val_maximo > 0 else 10
            if usl > 0:
                fig.add_hrect(y0=lsl, y1=warning_upper, fillcolor="green", opacity=0.15, line_width=0, layer="below")
                fig.add_hrect(y0=warning_upper, y1=usl, fillcolor="yellow", opacity=0.2, line_width=0, layer="below")
                fig.add_hrect(y0=usl, y1=tope, fillcolor="red", opacity=0.2, line_width=0, layer="below")
            else:
                fig.add_hrect(y0=0, y1=tope, fillcolor="red", opacity=0.2, line_width=0, layer="below")

            # Anotación último dato
            ultimo = df_grafica.iloc[-1]
            fig.add_annotation(x=ultimo['Fecha y hora'], y=ultimo[variable_sel], 
                               text=f"<b>ACTUAL: {ultimo[variable_sel]:.1f}</b>",
                               showarrow=True, arrowhead=2, ay=-40, bgcolor="white", font=dict(color="black"))

            fig.update_xaxes(type='date', tickformat="%d-%b %H:%M", tickangle=-45)
            fig.add_hline(y=usl, line_dash="dash", line_color="red", annotation_text=f"Límite Máx ({usl})")

            st.plotly_chart(fig, use_container_width=True)
        else:
            st.warning("No hay datos para esta combinación.")

    # TABLA
    st.markdown("---")
    st.subheader("📋 Registro de Mediciones")
    st.dataframe(df_final.sort_values('Fecha y hora', ascending=False), use_container_width=True)

    if st.button("🔄 Refrescar"):
        st.cache_data.clear()
        st.rerun()
else:
    st.error("Esperando sincronización de datos desde el Bot...")
